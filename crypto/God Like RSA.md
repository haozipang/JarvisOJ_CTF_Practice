...
# 思路分析
下载附件后得到三个文件，flag.enc和pubkey.pem应该都很熟悉了，可以从这些文件得到密文c和公钥<n，e>。
而另一个文件private.corrupted是私钥，且是被破坏了，那么就不能直接用私钥去解密flag.enc文件。
本题估计想考察的点就在于如何修复好这个私钥。搜索了一些信息后，在这里可以得到信息：https://www.40huo.cn/blog/rsa-private-key-recovery-and-oaep.html

代码粘贴如下，里面对应的信息替换对应的地方即可。
```
import re
...
将恢复的私钥和flag.enc导入环境用openssl进行破解，后面需要添加--oaep（怎么看出是用的oaep，暂时还不清楚）
```
